...
class SaleController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (Auth::user()->can('Manage Sales') && isset($sess) && !empty($sess) && count($sess) > 0) {
            ...
            if (!empty($details['customer'])) {
                ...
                $customerdetails = '<h2 class="h6 font-weight-normal"><b>' . ucfirst($details['customer']['name']) . '</b>' . '<p class="m-0 h6 font-weight-normal">' . $details['customer']['address'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $details['customer']['city'] . $details['customer']['state'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $details['customer']['zipcode'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $details['customer']['country'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' .$details['customer']['phone_number']  . '</p></h2>';
            } else {
            ...
            $userdetails = '<h2 class="h6"><b>' . ucfirst($details['user']['name']) . ' </b> <h2  class="h6 font-weight-normal">' . '<p class="m-0 h6 font-weight-normal">' . $settings['company_name'] . $settings['company_telephone'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $settings['company_address'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $settings['company_city'] . $settings['company_state'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $settings['company_zipcode'] . '</p>' . '<p class="m-0 h6 font-weight-normal">' . $settings['company_country'] . '</p></h2>';
            ...
            if (isset($sales) && !empty($sales) && count($sales) > 0) {
                ...
                if (count($result) > 0) {
                    ...
                    foreach ($sales as $key => $value) {
                        ...
                        $product->quantity    = $product->getTotalProductQuantity();
                        $product->save();
                    }
                    ...
                    if ($sale->customer != null) {
                        ...
                        if (isset($setting['twilio_sales_notification']) && $setting['twilio_sales_notification'] == 1) {
                            ...
                            // dd($sellproduct);
                            $uArr = [
                                // 'sale_invoice_id' => $this->invoiceSellNumber(),
                    ...
                    if ($webhook) {
                        $parameter = json_encode($sale);
                        // 1 parameter is  URL , 2 parameter is data , 3 parameter is method
                        $status = Utility::WebhookCall($webhook['url'], $parameter, $webhook['method']);
                        if ($status == true) {
                            // return redirect()->back()->with('success', __('Payment Completed Successfully!'));
                            return response()->json(
                                [
                                    'code' => 200,
    ...
    // change
            $totalquantity = $product_total = $totaltax = 0;
            foreach($selleditems as $key => $item)
            {
                //total quantity of all product
                $totalquantity += $st = $item->quantity;
                //total price of product before tax
                $p_total[] = $item->price * $item->quantity;
                //total of all product price before tax
                $product_total += $st = $item->price * $item->quantity;
                //totaltax of all product
                $subtotal_test = $item->price * $item->quantity;
                $taxes      = ($subtotal_test * $item->tax) / 100;
                $totaltax += $st = $taxes;
    ...
    //
            ...
            //Set your logo
            $sale_logo = Utility::getValByName('sale_logo');
            if (isset($sale_logo) && !empty($sale_logo)) {
                $img = \App\Models\Utility::get_file('uploads/invoice_logo/') . $sale_logo;
            } else {
            ...
            return view('sales.templates.' . $settings['sale_invoice_template'], compact('sale', 'color', 'font_color', 'settings', 'user', 'userdetails', 'customerdetails', 'img','subtotal','totalquantity','totaltax','product_total','p_total'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function previewSelledInvoice($template, $color)
    {
        ...
        $product_total = 600;
        $p_total = [200,200,200];
        // $tax_name = 'Tax'; //add in compact

        return view('sales.templates.' . $template, compact('sale', 'preview', 'color', 'font_color', 'settings', 'user', 'customerdetails', 'img','subtotal','totalquantity','totaltax','product_total','p_total'));
    }
    ...
    public function print(Request $request)
    {
        ...
        if (!empty($details['customer'])) {
            ...
            $customerdetails = ucfirst($details['customer']['name']) . $details['customer']['phone_number'] . $details['customer']['address'];
        } else {
            $customerdetails = ('Walk-in Customer');
        }
        ...
        return view('sales.print', compact('details', 'sales', 'customer'));
