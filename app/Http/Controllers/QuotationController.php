...
class QuotationController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->can('Create Quotations')) {
            ...
            if ($request->has('product') && $request->has('quantity')) {
                ...
                if (isset($setting['twilio_quotation_notification']) && $setting['twilio_quotation_notification'] == 1) {
                    ...
                    // dd($uArr);
                    Utility::send_twilio_msg($customer->phone_number,'new_quotation',$uArr);
                }
                
                // Webhook
                $module = 'New Quotation';
                $webhook =  Utility::webhookSetting($module);
                if ($webhook) {
                    $parameter = json_encode($q);
    ...
    //
            ...
            if (isset($quotation_logo) && !empty($quotation_logo)) {
                $img = \App\Models\Utility::get_file('uploads/invoice_logo/') . $quotation_logo;
            } else {
            ...
            return view('quotations.templates.' . $settings['quotation_invoice_template'], compact('quotation', 'color', 'font_color', 'settings', 'user', 'userdetails', 'customerdetails', 'img','subtotal','totalquantity','totaltax','product_total','p_total'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function previewQuotationInvoice($template, $color)
    {
        ...
        return view('quotations.templates.' . $template, compact('quotation', 'preview', 'color', 'font_color', 'settings', 'user', 'customerdetails', 'img','subtotal','totalquantity','totaltax','product_total','p_total'));
    }
