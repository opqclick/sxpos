...
use App\Exports\SaletaxreportExport;
...
class ReportController extends Controller
{
        ...
        if ((Auth::user()->can('Manage Purchases') || Auth::user()->can('Manage Sales')) && $request->ajax()) {
            ...
            foreach ($invoices->orderBy('id', 'DESC')->get() as $key => $invoice) {
                ...
                if ($data['vendors'] == 1) {
                    ...
                    $invoicearray[$key]['invoice_id'] = '
                        <a class="btn btn-outline-primary" href="#" data-ajax-popup="true" data-title="' . __('Purchase Invoice') . '" data-size="lg" data-url="' . route('show.sell.invoice', $invoice->id) . '" >' . Auth::user()->sellInvoiceNumberFormat($invoice->invoice_id) . '</a>
                       ';
                    $invoicearray[$key]['action']       = '
        ...
        if ($request->ajax()) {
            ...
            if ($productscount > 0) {
                ...
                foreach ($productObj->get() as $key => $product) {
                    ...
                    $product_quantity = $product->getTotalProductQuantity($data);
    ...
    public function productTaxAnalysisView(Request $request)
    {
        ...
        $display_status = '';
// dd(Auth::user()->isOwner(),Auth::user()->isUser());
        if (Auth::user()->isOwner()) {
    ...
    {
        try {
            $check_id = \Illuminate\Support\Facades\Crypt::decrypt($id);
        } catch (\Throwable $th) {
            return redirect()->back()->with('error', __('Purchase Invoice Not Found.'));
        }
    ...
    public function saleLink($id, Request $request)
    {
        try {
            ...
            return redirect()->back()->with('error', __('Sale Invoice Not Found.'));
        }
    ...
    public function stockanalysisexport()
    {
        $name = 'Stockanalysis_' . date('Y-m-d i:h:s');
        $data = Excel::download(new StockanalysisExport(), $name . '.xlsx'); ob_end_clean();

        return $data;
    }

    public function categoryreportexport()
    {
        $name = 'Categoryreport_' . date('Y-m-d i:h:s');
        $data = Excel::download(new CategoryreportExport(), $name . '.xlsx'); ob_end_clean();

        return $data;
    }

    public function brandreportexport()
    {
        $name = 'Brandreport_' . date('Y-m-d i:h:s');
        $data = Excel::download(new BrandreportExport(), $name . '.xlsx'); ob_end_clean();
    ...
    {
        $name = 'Taxpurchasesreport_' . date('Y-m-d i:h:s');
        $data = Excel::download(new PurchasetaxreportExport(), $name . '.xlsx'); ob_end_clean();
    ...
    {
        $name = 'Taxsalesreport' . date('Y-m-d i:h:s');
        $data = Excel::download(new SaletaxreportExport(), $name . '.xlsx'); ob_end_clean();

        return $data;
    }

    public function expensereportexport()
    {
        $name = 'Expensereport_' . date('Y-m-d i:h:s');
        $data = Excel::download(new Expensereportexport(), $name . '.xlsx'); ob_end_clean();
    ...
    {
        $name = 'Customerreport_' . date('Y-m-d i:h:s');
        $data = Excel::download(new CustomerreportExport(), $name . '.xlsx'); ob_end_clean();
    ...
    {
        $name = 'Vendorreport' . date('Y-m-d i:h:s');
        $data = Excel::download(new VendorreportExport(), $name . '.xlsx'); ob_end_clean();
