...
class PurchaseController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if(Auth::user()->can('Manage Purchases') && !empty($sess) && count($sess) > 0)
        {
            ...
            if(!empty($details['vendor']))
            {
                ...
                $vendordetails = '<h2 class="h6">' . ucfirst($details['vendor']['name']) . '</h2><h2  class="h6 font-weight-normal">' . '<p class="m-0">' . $details['vendor']['address'] . '</p>' . '<p class="m-0">' . $details['vendor']['city'] . $details['vendor']['state'] . '</p>' . '<p class="m-0">' .  $details['vendor']['zipcode'] . '</p>' . '<p class="m-0">' . $details['vendor']['country'] . '</p>' . '<p class="m-0">' . $details['vendor']['phone_number']. '</p>';
            }
            else
            {
            ...
            $userdetails = '<h2 class="h6 font-weight-normal">' . ucfirst($details['user']['name']) . '<h2 class="h6 font-weight-normal">' . '<p class="m-0">' . $settings['company_name'] . $settings['company_telephone'] . '</p>' . '<p class="m-0">' . $settings['company_address'] . '</p>' . '<p class="m-0">' . $settings['company_city'] . $settings['company_state'] . '</p>' . '<p class="m-0">' .$settings['company_zipcode'] . '</p>' . '<p class="m-0">' .  $settings['company_country'] . '</p>';
                ...
                else
                {
                    ...
                    foreach($purchases as $key => $value)
                    {
                        ...
                        //else{
                            ...
                            $purchaseditems->branch_id      = $purchase->branch_id;
                            $purchaseditems->cash_register_id = $purchase->cash_register_id;
                    ...
                    if($purchase->vendor != null)
                    {
                        ...
                        if (isset($setting['twilio_purchase_notification']) && $setting['twilio_purchase_notification'] == 1) {
                            // $msg = __("New User created by") . ' ' . \Auth::user()->name . '.';
                            
                            // $purchproduct = PurchasedItems::where('purchase_id', '=', $purchase->id)->first();
                            $vendor = Vendor::where('id', '=', $purchase->vendor_id)->first();

                            $uArr = [
                                // 'purchase_id' => $purchase->invoice_id,
                                // 'purchase_branch_id' => $purchase->branch_id,
                            ...
                            Utility::send_twilio_msg($vendor->phone_number,'new_purchase',$uArr);    //if any error so remove $to and ADD $user->phone
                        }
                    ...
                    if ($webhook) {
                        $parameter = json_encode($purchase);
    ...
    //
            ...
            if (isset($purchase_logo) && !empty($purchase_logo)) {
                $img = \App\Models\Utility::get_file('uploads/invoice_logo/') . $purchase_logo;
            } else {
            ...
            return view('purchases.templates.' . $settings['purchase_invoice_template'], compact('purchase', 'color', 'font_color', 'settings', 'user', 'userdetails', 'vendordetails', 'img','subtotal','totalquantity','totaltax','product_total','p_total'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function previewPurchasedInvoice($template, $color)
    {
        ...
        return view('purchases.templates.' . $template, compact('purchase', 'preview', 'font_color', 'color', 'settings', 'user', 'vendordetails', 'img','subtotal','totalquantity','totaltax','product_total','p_total'));
    }
