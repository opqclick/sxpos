    ...
    public function index()
    {
        if(Auth::user()->can('Manage Branch'))
        {
            $branches = DB::table('branches as b')
                          ->select('b.*', 'u.name as username', DB::raw("count(cr.branch_id) branchtotal"))
                          ->leftjoin('cash_registers as cr', 'cr.branch_id', '=', 'b.id')
                          ->leftjoin('users as u', 'u.id', '=', 'b.branch_manager')
                          ->where('b.created_by', '=', Auth::user()->getCreatedBy())
                          ->groupBy('b.id')
                          ->orderBy('b.id', 'DESC')
                          ->get();
