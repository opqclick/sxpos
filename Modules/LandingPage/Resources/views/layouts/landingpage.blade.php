...
@endphp
<!DOCTYPE html>
<html lang="en"  dir="@if(isset($setting['SITE_RTL'])){{$setting['SITE_RTL'] == 'on'?'rtl':''}}@else{{ Utility::getValByName('SITE_RTL') == 'on' ? 'rtl' : '' }}@endif">
...
<head>
    ...
    <!-- Favicon icon -->
    <link rel="icon" href="{{ $sup_logo.'/'. (isset($company_favicon) && !empty($company_favicon) ? $company_favicon : 'favicon.png').'?timestamp='.time() }}" type="image/x-icon" />
                ...
                <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                    <ul class="navbar-nav">
                        ...
                        @foreach (json_decode($settings['menubar_page']) as $key => $value)
                            ...
                            @if ($value->header == 'on' && $value->page_url != null)
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ url($value->page_url) }}" target="_blank">{{ $value->menubar_page_name }}</a>
                                </li>
                            @endif
                ...
                <div class="row g-0 gy-2 mt-4 align-items-center">
                    ...
                    <div class="col-xxl-9">
                        <div class="row gy-3 row-cols-9">
                            <div class="col-auto">
                                <img src="{{ $logo.'/'. $settings['home_logo'] .'?timestamp='.time() }}" alt="" class="img-fluid"
                                    style="width: 130px;">
                            </div>
                            <div class="col-auto">
                                <img src="{{ $logo.'/'. $settings['home_logo'] .'?timestamp='.time() }}" alt="" class="img-fluid"
